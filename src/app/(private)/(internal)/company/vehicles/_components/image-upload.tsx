'use client'
import { removeImage, uploadImage } from '@/common/actions/crud.action'
...
const ImageUpload = ({
	...
	const handleUpload = async (event: ChangeEvent<HTMLInputElement>) => {
		...
		img.onload = async () => {
			...
			await uploadImage(endpoint, id, formData)
				.then(resp => {
	...
	return (
		<section className='grid grid-cols-1 items-center gap-6 sm:grid-cols-[auto_1fr] sm:gap-3.5'>
			...
			<section className='flex flex-col gap-1.5'>
				<Label htmlFor='image-upload'>Imagen de {entity}</Label>
